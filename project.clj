(defproject my-website "0.1.0-SNAPSHOT"
  ...
  :clean-targets
    ^{:protect false}
      [:target-path :compile-path "resources/public/js" "dev-target"]

  :uberjar-name "my-website.jar"

  ;; Use `lein run` if you just want to start a HTTP server, without figwheel
  :main my-website.application

  ;; nREPL by default starts in the :main namespace, we want to start in `user`
  ;; because that's where our development helper functions like (go) and
  ;; (browser-repl) live.
  :repl-options
    {:init-ns user :timeout 240000}
  ...
  :cljsbuild
    {:builds
      [{:id "app"
        ...
        :figwheel {:on-jsload "my-website.system/reset"}

        :compiler
          {:main cljs.user
           :asset-path "js/compiled/out"
           :output-to "dev-target/public/js/compiled/my_website.js"
